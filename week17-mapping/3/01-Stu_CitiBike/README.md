...
A large part of this class is going to be devoted to working on a mini-project in Leaflet. We will utilise the Citi Bike API in order to retrieve the statuses and locations of all of the Citi Bike stations in New York. It's highly recommended to complete the **basic version** before attempting the **advanced version**.
...
* Create a second function `createMarkers` that will take `response` as an argument.
  ...
  * Use the response from a future D3 call loop through the stations and create a marker to represent each station.

  * Give each marker a popup to display the name and capacity of its station.

* In the `createMarkers` function, pass the result of the bike makers into the  `createmap` function as an `layerGroup`.
...
* Using the data retrieved from the second API call, try to add the following functionality:
  ...
  * Add a layer control and split the markers up into the following overlay layers:
    ...
    4. **Low Stations:** If a station has less than five bikes available.

    5. **Healthy Stations:** If a marker does not fall into any of the previous layer groups.

  * Utilise a Leaflet plugin to create different types of markers to represent each layer. The example shown below is using [Leaflet.extra-markers](https://github.com/coryasilva/Leaflet.ExtraMarkers), but you are free to use another plugin if you prefer.
